...
__all__ = ["detect_phishing", "predict_phishing"]
...
# ======================================================================
#
# The original version of this module exposed a single-argument function,
# ``predict_phishing(text) -> tuple[int, str]``, used for scoring either a
# raw URL or a block of email text and returning a bare (score, level)
# tuple. Downstream code (e.g. gmail_scanner_page.py) may still import
# and call that name/signature directly.
#
# This wrapper preserves that exact call contract on top of the new
# hybrid engine, so existing call sites keep working unmodified, while
# new code should prefer calling detect_phishing(...) directly for the
# full structured result (reasons, detected_features, recommendation).

#: Legacy callers expect only three risk labels. The new engine's five
#: levels are collapsed onto the old three-level scale for compatibility.
...
#: A bare string with no whitespace but containing a dot (e.g.
#: "http://evil.com/login" or "evil.com/login") is treated as a URL for
#: legacy single-argument calls; anything else is treated as email text
#: (checked against both subject- and body-style rules for best coverage).
_URL_LIKE_RE = re.compile(r'^\s*(?:[a-zA-Z][\w+.-]*://|(?:[\w-]+\.)+[a-zA-Z]{2,}(?:[/?#]|$))')


def predict_phishing(text: str) -> tuple[int, str]:
    """Legacy-compatible single-argument phishing check.

    .. deprecated::
        Prefer :func:`detect_phishing`, which accepts separate
        ``sender``/``subject``/``body``/``urls`` fields and returns a
        full structured result (reasons, detected_features,
        recommendation) instead of a bare tuple.

    Parameters
    ----------
    text:
        Either a single URL or a block of email text (subject/body).
        URL-shaped input (starts with a scheme, or looks like a bare
        domain/path with no spaces) is routed through the URL analyzer;
        everything else is routed through the subject/body analyzers.

    Returns
    -------
    tuple[int, str]
        ``(score, risk_level)`` where ``risk_level`` is one of
        ``"Low"``, ``"Medium"``, ``"High"`` -- matching the original
        three-level scale used by earlier callers.
    """
    text = _safe_str(text)
    if not text:
        return 0, "Low"
    ...
    if _URL_LIKE_RE.match(text):
        result = detect_phishing(sender="", subject="", body="", urls=[text])
    else:
        # Treat the same text as both subject and body so keyword rules
        # from either category can fire, matching the old function's
        # single shared keyword-scan behavior.
        result = detect_phishing(sender="", subject=text, body=text, urls=[])

    legacy_level = _LEGACY_LEVEL_MAP[result["risk_level"]]
    return result["score"], legacy_level
